...
- **加密托管**：软件许可证密钥以 AES-GCM 加密存储（密钥来自 `security.config_encryption_key`），明文不落库、不在列表/详情接口暴露。
- **许可证台账**：管理员/资产管理员在「软件许可」页面新增、编辑、删除许可证（软件名、厂商、类型 commercial/open-source/subscription/other、授权数量/已用数量、到期日、购买日期、可选关联资产）。
- **到期提醒**：进程内许可扫描器每天检查一次，对到期日在未来 30 天内（含已过期）的许可证发送**邮件**（收件人为系统管理员）与 **IM 群通知**；前端列表对已过期红色、30 天内「即将到期」黄色高亮。
- **查看密钥**：查看许可证密钥明文需二次确认并写入操作审计。
- **权限**：许可证接口仅 admin/asset_manager 可访问：`/licenses` CRUD、`/licenses/:id/reveal` 查看密钥明文。

## v2.21 软件许可增强（导入导出 / 附件 / 到期续费工单）

- **批量导入导出**：「软件许可」页支持 CSV/XLSX 模板下载与批量导入（按「软件名+厂商」匹配，已存在则更新、不存在则新增，密钥必填并加密存储；关联资产按主机名/IP 解析）；批量导出 CSV 不包含密钥明文。
- **许可附件**：许可证支持上传授权书/合同扫描件等附件（存储于 `attachment_dir/licenses/<id>/`，单文件 ≤ 20MB），可下载/删除并写审计；接口 `/licenses/:id/attachments`。
- **到期自动续费工单**：许可到期扫描器为 30 天内到期/已过期许可证自动生成「软件许可续费」草稿工单（类型 `license_renew`，申请人取管理员，关联资产与许可），同一许可存在未关闭/未取消续费工单时不重复生成；管理员在「流程配置」页为该类型配置审批流程后即可走既有审批流。
- **工单统计**：工单报表新增「软件许可续费」类型标签。
...
- **批量打印标签**：资产列表勾选资产后「打印标签」，打开打印页按网格展示二维码 + 编号/主机名/IP/类型，`@media print` 排版支持浏览器打印。
