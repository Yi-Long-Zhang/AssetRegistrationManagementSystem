// Package docs Code generated by swaggo/swag. DO NOT EDIT
package docs

import "github.com/swaggo/swag"

const docTemplate = `{
    "schemes": {{ marshal .Schemes }},
    "swagger": "2.0",
    "info": {
        "description": "{{escape .Description}}",
        "title": "{{.Title}}",
        "contact": {},
        "version": "{{.Version}}"
    },
    "host": "{{.Host}}",
    "basePath": "{{.BasePath}}",
    "paths": {
        "/ad/config": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "settings"
                ],
                "summary": "AD 配置详情",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "settings"
                ],
                "summary": "保存 AD 配置",
                "parameters": [
                    {
                        "description": "AD 配置",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.adConfigRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/ad/import-user": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "settings"
                ],
                "summary": "导入 AD 用户",
                "parameters": [
                    {
                        "description": "导入参数",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.adImportRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.User"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/ad/lookup-user": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "settings"
                ],
                "summary": "查询 AD 用户",
                "parameters": [
                    {
                        "description": "查询参数",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.adLookupRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_service.ADUserInfo"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/ad/test": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "settings"
                ],
                "summary": "测试 AD 连接",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/assets": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "assets"
                ],
                "summary": "资产列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "关键词",
                        "name": "q",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "pageSize",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "assets"
                ],
                "summary": "创建资产",
                "parameters": [
                    {
                        "description": "资产信息",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.assetRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Asset"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/assets/batch-delete": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "assets"
                ],
                "summary": "批量删除资产",
                "parameters": [
                    {
                        "description": "资产 ID 列表",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.batchDeleteRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/assets/export": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "text/csv"
                ],
                "tags": [
                    "assets"
                ],
                "summary": "导出资产 CSV",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "file"
                        }
                    }
                }
            }
        },
        "/assets/import": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "multipart/form-data"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "assets"
                ],
                "summary": "导入资产",
                "parameters": [
                    {
                        "type": "file",
                        "description": "CSV 或 XLSX 文件",
                        "name": "file",
                        "in": "formData",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/assets/stats": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "assets"
                ],
                "summary": "资产统计",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/assets/stats/export": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "导出资产统计汇总为 CSV（在线状态/类型/TOP 端口/TOP 服务）",
                "produces": [
                    "text/csv"
                ],
                "tags": [
                    "assets"
                ],
                "summary": "资产统计报表导出",
                "responses": {
                    "200": {
                        "description": "CSV 文件",
                        "schema": {
                            "type": "string"
                        }
                    }
                }
            }
        },
        "/assets/template": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/octet-stream"
                ],
                "tags": [
                    "assets"
                ],
                "summary": "下载资产导入模板",
                "parameters": [
                    {
                        "type": "string",
                        "description": "模板格式 csv 或 xlsx",
                        "name": "format",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "file"
                        }
                    }
                }
            }
        },
        "/assets/{id}": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "assets"
                ],
                "summary": "资产详情",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "资产 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Asset"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "assets"
                ],
                "summary": "更新资产",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "资产 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "资产信息",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.assetRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Asset"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "assets"
                ],
                "summary": "删除资产",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "资产 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/assets/{id}/history": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "assets"
                ],
                "summary": "资产变更历史",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "资产 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.AssetSnapshot"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/assets/{id}/retire": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "将资产状态置为退役（retired），保留快照与审计记录",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "assets"
                ],
                "summary": "资产退役归档",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "资产 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Asset"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/audit-logs": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "分页查询操作审计日志（admin），支持 entity/action 过滤",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "system"
                ],
                "summary": "操作审计日志",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码（默认 1）",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页条数（默认 20，最大 200）",
                        "name": "pageSize",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实体（asset/ticket/user/...）",
                        "name": "entity",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "动作（create/update/delete/alert/...）",
                        "name": "action",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/auth/change-password": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "修改当前用户密码",
                "parameters": [
                    {
                        "description": "密码修改参数",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.changePasswordDocRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/auth/login": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "用户登录",
                "parameters": [
                    {
                        "description": "登录参数",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.loginRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/auth/logout": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "用户退出",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/auth/me": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "当前用户",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/auth/reauth": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "刷新当前会话的敏感操作认证有效期",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "二次认证",
                "parameters": [
                    {
                        "description": "当前密码",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.reauthenticateDocRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/auth/sessions": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "当前用户会话列表",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/auth/sessions/revoke-all": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "撤销当前用户的全部服务端会话",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "全端退出",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/auth/sessions/{id}": {
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "撤销指定会话",
                "parameters": [
                    {
                        "type": "string",
                        "description": "会话 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/backups": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "查询全部数据库备份文件（admin）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "backups"
                ],
                "summary": "备份列表",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "生成一份 SQLite 一致性快照备份（admin）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "backups"
                ],
                "summary": "创建备份",
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_service.BackupInfo"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/backups/{name}": {
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除指定备份文件（admin）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "backups"
                ],
                "summary": "删除备份",
                "parameters": [
                    {
                        "type": "string",
                        "description": "备份文件名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/backups/{name}/download": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "下载指定备份数据库文件（admin）",
                "produces": [
                    "application/octet-stream"
                ],
                "tags": [
                    "backups"
                ],
                "summary": "下载备份",
                "parameters": [
                    {
                        "type": "string",
                        "description": "备份文件名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "file"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/backups/{name}/restore": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "校验备份并标记待恢复，重启后端后生效（admin）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "backups"
                ],
                "summary": "恢复备份",
                "parameters": [
                    {
                        "type": "string",
                        "description": "备份文件名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/backups/{name}/verify": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "backups"
                ],
                "summary": "校验完整备份",
                "parameters": [
                    {
                        "type": "string",
                        "description": "备份文件名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_service.BackupInfo"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/credentials": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "查询全部凭据（admin/asset_manager），不含明文",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "credentials"
                ],
                "summary": "凭据列表",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "创建凭据，secret 以 AES-GCM 加密存储（admin/asset_manager）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "credentials"
                ],
                "summary": "创建凭据",
                "parameters": [
                    {
                        "description": "凭据",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.credentialRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Credential"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/credentials/{id}": {
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "更新凭据元信息，secret 非空时重新加密存储（admin/asset_manager）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "credentials"
                ],
                "summary": "更新凭据",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "凭据 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "凭据",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.credentialRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Credential"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除凭据（admin/asset_manager）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "credentials"
                ],
                "summary": "删除凭据",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "凭据 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/credentials/{id}/reveal": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "解密返回明文 secret，写入查看审计（admin/asset_manager）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "credentials"
                ],
                "summary": "查看凭据明文",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "凭据 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/discovery/rules": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "discovery"
                ],
                "summary": "发现规则列表",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.DiscoveryRule"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "discovery"
                ],
                "summary": "创建发现规则",
                "parameters": [
                    {
                        "description": "发现规则",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.discoveryRuleRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.DiscoveryRule"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/discovery/rules/{id}": {
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "discovery"
                ],
                "summary": "更新发现规则",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "规则 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "发现规则",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.discoveryRuleRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.DiscoveryRule"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "discovery"
                ],
                "summary": "删除发现规则",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "规则 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/discovery/rules/{id}/run": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "discovery"
                ],
                "summary": "手动触发发现任务",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "规则 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "202": {
                        "description": "Accepted",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.DiscoveryRun"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/discovery/rules/{id}/test": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "discovery"
                ],
                "summary": "试跑发现规则",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "规则 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/discovery/runs": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "discovery"
                ],
                "summary": "运行记录列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "规则 ID",
                        "name": "ruleId",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "状态",
                        "name": "status",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/discovery/runs/{id}": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "discovery"
                ],
                "summary": "运行记录详情",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "运行 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.DiscoveryRun"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/discovery/runs/{id}/adopt": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "discovery"
                ],
                "summary": "纳管新发现主机",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "运行 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "主机 ID 列表",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.discoveryHostActionRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/discovery/runs/{id}/apply": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "discovery"
                ],
                "summary": "应用发现变更到资产",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "运行 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "主机 ID 列表",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.discoveryHostActionRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/discovery/stats/services": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "按开放端口聚合资产数量（含 -sV 识别的服务名），按数量降序返回",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "discovery"
                ],
                "summary": "端口/服务矩阵",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "返回条数（默认 50）",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/internal_httpapi.serviceStat"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/discovery/stats/subnets": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "按网段聚合资产数量（subnet 字段优先，未填时按 IP /16 兜底）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "discovery"
                ],
                "summary": "资产网段分布",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/internal_httpapi.subnetStat"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/discovery/stats/trend": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "按天聚合各发现运行的变更统计（默认最近 14 天，可用 days 参数调整）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "discovery"
                ],
                "summary": "发现趋势统计",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "统计天数（默认 14）",
                        "name": "days",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/internal_httpapi.discoveryTrendItem"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/im/callback": {
            "post": {
                "description": "接收 IM 平台事件回调（飞书 challenge 验证；approve/reject 交互动作经用户绑定鉴权后流转工单）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "system"
                ],
                "summary": "IM 平台回调",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/inspection/rules": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "查询定期巡检规则（admin）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "inspection"
                ],
                "summary": "巡检规则列表",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "创建定期巡检规则（admin）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "inspection"
                ],
                "summary": "新增巡检规则",
                "parameters": [
                    {
                        "description": "巡检规则",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.inspectionRuleRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.InspectionRule"
                        }
                    }
                }
            }
        },
        "/inspection/rules/{id}": {
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "更新定期巡检规则（admin）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "inspection"
                ],
                "summary": "更新巡检规则",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "规则 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "巡检规则",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.inspectionRuleRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.InspectionRule"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除定期巡检规则（admin）",
                "tags": [
                    "inspection"
                ],
                "summary": "删除巡检规则",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "规则 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/inspection/rules/{id}/test": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "立即生成一张巡检工单（admin）",
                "tags": [
                    "inspection"
                ],
                "summary": "试运行巡检规则",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "规则 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Ticket"
                        }
                    }
                }
            }
        },
        "/licenses": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "software-licenses"
                ],
                "summary": "软件许可证列表",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "software-licenses"
                ],
                "summary": "创建软件许可证",
                "parameters": [
                    {
                        "description": "软件许可证",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.softwareLicenseRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.SoftwareLicense"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/licenses/export": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "text/csv"
                ],
                "tags": [
                    "software-licenses"
                ],
                "summary": "导出软件许可证 CSV",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "file"
                        }
                    }
                }
            }
        },
        "/licenses/import": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "multipart/form-data"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "software-licenses"
                ],
                "summary": "导入软件许可证",
                "parameters": [
                    {
                        "type": "file",
                        "description": "CSV 或 XLSX 文件",
                        "name": "file",
                        "in": "formData",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/licenses/template": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/octet-stream"
                ],
                "tags": [
                    "software-licenses"
                ],
                "summary": "下载许可证导入模板",
                "parameters": [
                    {
                        "type": "string",
                        "description": "模板格式 csv 或 xlsx",
                        "name": "format",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "file"
                        }
                    }
                }
            }
        },
        "/licenses/{id}": {
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "software-licenses"
                ],
                "summary": "更新软件许可证",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "许可证 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "软件许可证",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.softwareLicenseRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.SoftwareLicense"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "software-licenses"
                ],
                "summary": "删除软件许可证",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "许可证 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/licenses/{id}/attachments": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "software-licenses"
                ],
                "summary": "许可证附件列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "许可证 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "multipart/form-data"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "software-licenses"
                ],
                "summary": "上传许可证附件",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "许可证 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "file",
                        "description": "附件",
                        "name": "file",
                        "in": "formData",
                        "required": true
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.LicenseAttachment"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/licenses/{id}/attachments/{attachmentId}": {
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "software-licenses"
                ],
                "summary": "删除许可证附件",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "许可证 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "附件 ID",
                        "name": "attachmentId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/licenses/{id}/attachments/{attachmentId}/download": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/octet-stream"
                ],
                "tags": [
                    "software-licenses"
                ],
                "summary": "下载许可证附件",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "许可证 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "附件 ID",
                        "name": "attachmentId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "file"
                        }
                    }
                }
            }
        },
        "/licenses/{id}/reveal": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "software-licenses"
                ],
                "summary": "查看许可证密钥明文",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "许可证 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/racks": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "查询机柜列表（可按 roomId 过滤，admin/asset_manager）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "racks"
                ],
                "summary": "机柜列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "机房 ID",
                        "name": "roomId",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "创建机柜（admin/asset_manager）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "racks"
                ],
                "summary": "新增机柜",
                "parameters": [
                    {
                        "description": "机柜",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.rackRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Rack"
                        }
                    }
                }
            }
        },
        "/racks/{id}": {
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "更新机柜信息（admin/asset_manager）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "racks"
                ],
                "summary": "更新机柜",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "机柜 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "机柜",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.rackRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Rack"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除机柜（admin/asset_manager）",
                "tags": [
                    "racks"
                ],
                "summary": "删除机柜",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "机柜 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/roles": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "角色列表",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "403": {
                        "description": "Forbidden",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/rooms": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "查询机房及机柜列表（admin/asset_manager）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "rooms"
                ],
                "summary": "机房列表",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "创建机房（admin/asset_manager）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "rooms"
                ],
                "summary": "新增机房",
                "parameters": [
                    {
                        "description": "机房",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.roomRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.DatacenterRoom"
                        }
                    }
                }
            }
        },
        "/rooms/{id}": {
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "更新机房信息（admin/asset_manager）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "rooms"
                ],
                "summary": "更新机房",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "机房 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "机房",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.roomRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.DatacenterRoom"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除机房及其机柜（admin/asset_manager）",
                "tags": [
                    "rooms"
                ],
                "summary": "删除机房",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "机房 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/settings/im": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "读取钉钉/企微/飞书群机器人通知配置（secret 不回显）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "settings"
                ],
                "summary": "IM 通知配置",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "保存钉钉/企微/飞书群机器人通知配置（启用时必须填写 webhook；secret 加密存储，留空不修改）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "settings"
                ],
                "summary": "保存 IM 通知配置",
                "parameters": [
                    {
                        "description": "IM 配置",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.imConfigRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/settings/im/bindings": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "查询 IM 用户与系统用户映射（用于 IM 回调鉴权）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "settings"
                ],
                "summary": "IM 用户绑定列表",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.IMBinding"
                            }
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "建立 IM 用户与系统用户映射（同一用户仅保留一个平台绑定）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "settings"
                ],
                "summary": "新增/更新 IM 绑定",
                "parameters": [
                    {
                        "description": "绑定信息",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.imBindingRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.IMBinding"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/settings/im/bindings/{userId}": {
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除指定用户的 IM 绑定",
                "tags": [
                    "settings"
                ],
                "summary": "删除 IM 绑定",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "系统用户 ID",
                        "name": "userId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                }
            }
        },
        "/settings/im/callback": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "读取钉钉/企微/飞书自建应用回调验签配置（敏感字段不回显）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "settings"
                ],
                "summary": "IM 回调验签配置",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "保存钉钉/企微/飞书自建应用回调验签配置（敏感字段加密存储，留空不修改）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "settings"
                ],
                "summary": "保存 IM 回调验签配置",
                "parameters": [
                    {
                        "description": "回调验签配置",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.imCallbackConfigRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/settings/im/test": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "使用当前配置发送一条测试消息到群机器人",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "settings"
                ],
                "summary": "测试 IM 通知",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/settings/mail": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "settings"
                ],
                "summary": "邮件配置详情",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "settings"
                ],
                "summary": "保存邮件配置",
                "parameters": [
                    {
                        "description": "邮件配置",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.mailConfigRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/settings/mail/test": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "settings"
                ],
                "summary": "测试邮件发送",
                "parameters": [
                    {
                        "description": "测试参数",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.mailTestRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/stocktakes": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "查询盘点单列表（admin/asset_manager）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "stocktakes"
                ],
                "summary": "盘点单列表",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "按范围快照资产并生成盘点明细（admin/asset_manager）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "stocktakes"
                ],
                "summary": "创建盘点单",
                "parameters": [
                    {
                        "description": "盘点单",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.createStocktakeRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.StocktakeTask"
                        }
                    }
                }
            }
        },
        "/stocktakes/{id}": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "查询盘点单与明细（admin/asset_manager）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "stocktakes"
                ],
                "summary": "盘点单详情",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "盘点单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.StocktakeTask"
                        }
                    }
                }
            }
        },
        "/stocktakes/{id}/close": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "关闭盘点单并产出差异统计（admin/asset_manager）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "stocktakes"
                ],
                "summary": "关闭盘点单",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "盘点单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/stocktakes/{id}/export": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "导出盘点明细为 CSV（含盘亏资产，admin/asset_manager）",
                "produces": [
                    "text/csv"
                ],
                "tags": [
                    "stocktakes"
                ],
                "summary": "导出盘点差异报告",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "盘点单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "CSV 文件",
                        "schema": {
                            "type": "string"
                        }
                    }
                }
            }
        },
        "/stocktakes/{id}/items/{itemId}": {
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "标记盘点项为 matched/missing（admin/asset_manager）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "stocktakes"
                ],
                "summary": "核对盘点项",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "盘点单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "盘点项 ID",
                        "name": "itemId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "核对结果",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.stocktakeItemRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.StocktakeItem"
                        }
                    }
                }
            }
        },
        "/tasks": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tasks"
                ],
                "summary": "后台任务列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "任务类型",
                        "name": "kind",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "任务状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "pageSize",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "403": {
                        "description": "Forbidden",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/tasks/{id}": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tasks"
                ],
                "summary": "后台任务详情",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "任务 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.BackgroundTask"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/tasks/{id}/acknowledge": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tasks"
                ],
                "summary": "确认后台任务告警",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "任务 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/tasks/{id}/retry": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tasks"
                ],
                "summary": "重试后台任务",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "任务 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.BackgroundTask"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/tickets": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "工单列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "视图 todo/submitted/all",
                        "name": "view",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "创建工单",
                "parameters": [
                    {
                        "description": "工单信息",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.ticketRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Ticket"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/tickets/archives/download": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/zip"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "批量下载工单归档 ZIP",
                "parameters": [
                    {
                        "description": "工单 ID 列表",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.ticketArchiveBatchRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "file"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/tickets/stats": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "按类型/状态/优先级分布、月度趋势与 SLA 达标率统计（admin）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "工单统计报表",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.TicketStats"
                        }
                    }
                }
            }
        },
        "/tickets/stats/export": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "导出工单统计汇总为 CSV（状态/类型/优先级/SLA 达标率/月度趋势，admin）",
                "produces": [
                    "text/csv"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "导出工单统计报表",
                "responses": {
                    "200": {
                        "description": "CSV 文件",
                        "schema": {
                            "type": "string"
                        }
                    }
                }
            }
        },
        "/tickets/{id}": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "工单详情",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "工单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Ticket"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "更新工单",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "工单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "工单信息",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.ticketRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Ticket"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/tickets/{id}/accept": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "验收通过并归档",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "工单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "验收结果",
                        "name": "body",
                        "in": "body",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.ticketActionRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Ticket"
                        }
                    }
                }
            }
        },
        "/tickets/{id}/approve": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "审批通过",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "工单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "处理意见",
                        "name": "body",
                        "in": "body",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.ticketActionRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Ticket"
                        }
                    },
                    "403": {
                        "description": "Forbidden",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/tickets/{id}/archive/download": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/pdf"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "下载工单归档 PDF",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "工单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "file"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/tickets/{id}/attachments": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "工单附件列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "工单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "multipart/form-data"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "上传工单附件",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "工单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "file",
                        "description": "附件",
                        "name": "file",
                        "in": "formData",
                        "required": true
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketAttachment"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/tickets/{id}/attachments/{attachmentId}/download": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/octet-stream"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "下载工单附件",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "工单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "附件 ID",
                        "name": "attachmentId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "file"
                        }
                    }
                }
            }
        },
        "/tickets/{id}/cancel": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "取消工单",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "工单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "取消原因",
                        "name": "body",
                        "in": "body",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.ticketActionRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Ticket"
                        }
                    }
                }
            }
        },
        "/tickets/{id}/comments": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "工单评论列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "工单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "创建工单评论",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "工单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "评论",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.ticketCommentRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketComment"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/tickets/{id}/complete": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "执行完成",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "工单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "执行结果",
                        "name": "body",
                        "in": "body",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.ticketActionRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Ticket"
                        }
                    }
                }
            }
        },
        "/tickets/{id}/reject": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "审批驳回",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "工单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "处理意见",
                        "name": "body",
                        "in": "body",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.ticketActionRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Ticket"
                        }
                    },
                    "403": {
                        "description": "Forbidden",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/tickets/{id}/start": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "开始执行",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "工单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "处理意见",
                        "name": "body",
                        "in": "body",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.ticketActionRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Ticket"
                        }
                    }
                }
            }
        },
        "/tickets/{id}/submit": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tickets"
                ],
                "summary": "提交工单",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "工单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "处理意见",
                        "name": "body",
                        "in": "body",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.ticketActionRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Ticket"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/users": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "用户列表",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "创建用户",
                "parameters": [
                    {
                        "description": "用户信息",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.userRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.User"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/users/{id}": {
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "更新用户",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "用户 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "用户信息",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.userRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.User"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/workflows": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "workflows"
                ],
                "summary": "流程配置列表",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/workflows/{type}": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "workflows"
                ],
                "summary": "工单类型流程配置",
                "parameters": [
                    {
                        "type": "string",
                        "description": "工单类型",
                        "name": "type",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketWorkflow"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "workflows"
                ],
                "summary": "保存工单类型流程配置",
                "parameters": [
                    {
                        "type": "string",
                        "description": "工单类型",
                        "name": "type",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "流程配置",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.workflowRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketWorkflow"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        },
        "/workflows/{type}/enable": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "workflows"
                ],
                "summary": "启用工单类型流程",
                "parameters": [
                    {
                        "type": "string",
                        "description": "工单类型",
                        "name": "type",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketWorkflow"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_httpapi.errorResponse"
                        }
                    }
                }
            }
        }
    },
    "definitions": {
        "asset-registration-management-system_backend_internal_model.Asset": {
            "type": "object",
            "properties": {
                "additionalIPs": {
                    "description": "关联 IP 列表，逗号分隔（多网卡）",
                    "type": "string"
                },
                "appVersion": {
                    "type": "string"
                },
                "assetNo": {
                    "type": "string"
                },
                "assetType": {
                    "type": "string"
                },
                "businessSystem": {
                    "type": "string"
                },
                "cpu": {
                    "type": "string"
                },
                "createdAt": {
                    "type": "string"
                },
                "department": {
                    "type": "string"
                },
                "discoveredAt": {
                    "type": "string"
                },
                "disk": {
                    "type": "string"
                },
                "environment": {
                    "type": "string"
                },
                "hostname": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "ip": {
                    "type": "string"
                },
                "lastSeenAt": {
                    "type": "string"
                },
                "location": {
                    "type": "string"
                },
                "macAddress": {
                    "type": "string"
                },
                "maintenanceVendor": {
                    "type": "string"
                },
                "managementIp": {
                    "type": "string"
                },
                "manufacturer": {
                    "type": "string"
                },
                "memory": {
                    "type": "string"
                },
                "model": {
                    "type": "string"
                },
                "onlineDate": {
                    "type": "string"
                },
                "onlineStatus": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.AssetOnlineStatus"
                },
                "openPorts": {
                    "type": "string"
                },
                "os": {
                    "type": "string"
                },
                "osVersion": {
                    "type": "string"
                },
                "owner": {
                    "type": "string"
                },
                "purchaseDate": {
                    "type": "string"
                },
                "rack": {
                    "type": "string"
                },
                "rackPosition": {
                    "type": "string"
                },
                "remark": {
                    "type": "string"
                },
                "runningServices": {
                    "type": "string"
                },
                "sequenceNo": {
                    "type": "string"
                },
                "serialNo": {
                    "type": "string"
                },
                "status": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.AssetStatus"
                },
                "subnet": {
                    "type": "string"
                },
                "updatedAt": {
                    "type": "string"
                },
                "warrantyExpireDate": {
                    "type": "string"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.AssetOnlineStatus": {
            "type": "string",
            "enum": [
                "online",
                "offline",
                "unknown"
            ],
            "x-enum-varnames": [
                "AssetOnlineStatusOnline",
                "AssetOnlineStatusOffline",
                "AssetOnlineStatusUnknown"
            ]
        },
        "asset-registration-management-system_backend_internal_model.AssetSnapshot": {
            "type": "object",
            "properties": {
                "assetId": {
                    "type": "integer"
                },
                "changeType": {
                    "type": "string"
                },
                "createdAt": {
                    "type": "string"
                },
                "createdBy": {
                    "type": "integer"
                },
                "diffSummary": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "source": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.SnapshotSource"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.AssetStatus": {
            "type": "string",
            "enum": [
                "pending",
                "in_use",
                "maintenance",
                "retired",
                "decommissioned"
            ],
            "x-enum-varnames": [
                "AssetStatusPending",
                "AssetStatusInUse",
                "AssetStatusMaintenance",
                "AssetStatusRetired",
                "AssetStatusDecommission"
            ]
        },
        "asset-registration-management-system_backend_internal_model.BackgroundTask": {
            "type": "object",
            "properties": {
                "acknowledgedAt": {
                    "type": "string"
                },
                "acknowledgedBy": {
                    "type": "integer"
                },
                "attempts": {
                    "type": "integer"
                },
                "createdAt": {
                    "type": "string"
                },
                "error": {
                    "type": "string"
                },
                "finishedAt": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "kind": {
                    "type": "string"
                },
                "maxAttempts": {
                    "type": "integer"
                },
                "result": {
                    "type": "string"
                },
                "scheduledAt": {
                    "type": "string"
                },
                "source": {
                    "type": "string"
                },
                "startedAt": {
                    "type": "string"
                },
                "status": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.BackgroundTaskStatus"
                },
                "uniqueKey": {
                    "type": "string"
                },
                "updatedAt": {
                    "type": "string"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.BackgroundTaskStatus": {
            "type": "string",
            "enum": [
                "queued",
                "running",
                "succeeded",
                "failed"
            ],
            "x-enum-varnames": [
                "BackgroundTaskQueued",
                "BackgroundTaskRunning",
                "BackgroundTaskSucceeded",
                "BackgroundTaskFailed"
            ]
        },
        "asset-registration-management-system_backend_internal_model.ChangeRiskLevel": {
            "type": "string",
            "enum": [
                "low",
                "high"
            ],
            "x-enum-varnames": [
                "ChangeRiskLow",
                "ChangeRiskHigh"
            ]
        },
        "asset-registration-management-system_backend_internal_model.Credential": {
            "type": "object",
            "properties": {
                "asset": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Asset"
                },
                "assetId": {
                    "type": "integer"
                },
                "createdAt": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "lastAccessedAt": {
                    "type": "string"
                },
                "name": {
                    "description": "凭据用途名，如 \"root 登录\"",
                    "type": "string"
                },
                "remark": {
                    "type": "string"
                },
                "type": {
                    "description": "ssh/rdp/database/app/other",
                    "type": "string"
                },
                "updatedAt": {
                    "type": "string"
                },
                "username": {
                    "type": "string"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.DatacenterRoom": {
            "type": "object",
            "properties": {
                "createdAt": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "location": {
                    "description": "位置/地址",
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "racks": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Rack"
                    }
                },
                "remark": {
                    "type": "string"
                },
                "updatedAt": {
                    "type": "string"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.DiscoveredHost": {
            "type": "object",
            "properties": {
                "adopted": {
                    "type": "boolean"
                },
                "applied": {
                    "type": "boolean"
                },
                "changeRisk": {
                    "description": "变更风险级别：low=可自动应用 / high=需人工确认",
                    "allOf": [
                        {
                            "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.ChangeRiskLevel"
                        }
                    ]
                },
                "changeType": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.DiscoveryChangeType"
                },
                "createdAt": {
                    "type": "string"
                },
                "diffSummary": {
                    "type": "string"
                },
                "hostname": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "ip": {
                    "type": "string"
                },
                "mac": {
                    "type": "string"
                },
                "matchedAsset": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Asset"
                },
                "matchedAssetId": {
                    "type": "integer"
                },
                "openPorts": {
                    "type": "string"
                },
                "os": {
                    "type": "string"
                },
                "runId": {
                    "type": "integer"
                },
                "services": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.DiscoveryChangeType": {
            "type": "string",
            "enum": [
                "new",
                "changed",
                "offline",
                "online",
                "none"
            ],
            "x-enum-varnames": [
                "DiscoveryChangeNew",
                "DiscoveryChangeChanged",
                "DiscoveryChangeOffline",
                "DiscoveryChangeOnline",
                "DiscoveryChangeNone"
            ]
        },
        "asset-registration-management-system_backend_internal_model.DiscoveryRule": {
            "type": "object",
            "properties": {
                "autoAdopt": {
                    "type": "boolean"
                },
                "autoApply": {
                    "type": "boolean"
                },
                "autoTicket": {
                    "description": "高风险变更自动生成变更工单",
                    "type": "boolean"
                },
                "createdAt": {
                    "type": "string"
                },
                "enabled": {
                    "type": "boolean"
                },
                "id": {
                    "type": "integer"
                },
                "incremental": {
                    "description": "增量扫描：仅重扫上次发现的存活主机",
                    "type": "boolean"
                },
                "intervalMinutes": {
                    "type": "integer"
                },
                "lastRunAt": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "ports": {
                    "description": "端口列表，如 \"22,80,443\"；空=使用默认端口",
                    "type": "string"
                },
                "probePorts": {
                    "description": "两阶段扫描探活端口；空=使用配置默认",
                    "type": "string"
                },
                "scanWindowEnd": {
                    "description": "扫描时段结束 \"HH:MM\"，空=全天",
                    "type": "string"
                },
                "scanWindowStart": {
                    "description": "扫描时段开始 \"HH:MM\"，空=全天",
                    "type": "string"
                },
                "serviceDetect": {
                    "type": "boolean"
                },
                "targets": {
                    "description": "CIDR/IP 列表，逗号分隔",
                    "type": "string"
                },
                "updatedAt": {
                    "type": "string"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.DiscoveryRun": {
            "type": "object",
            "properties": {
                "changedCount": {
                    "type": "integer"
                },
                "error": {
                    "type": "string"
                },
                "finishedAt": {
                    "type": "string"
                },
                "hosts": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.DiscoveredHost"
                    }
                },
                "id": {
                    "type": "integer"
                },
                "newCount": {
                    "type": "integer"
                },
                "offlineCount": {
                    "type": "integer"
                },
                "onlineCount": {
                    "type": "integer"
                },
                "rule": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.DiscoveryRule"
                },
                "ruleId": {
                    "type": "integer"
                },
                "startedAt": {
                    "type": "string"
                },
                "status": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.DiscoveryRunStatus"
                },
                "totalHosts": {
                    "type": "integer"
                },
                "trigger": {
                    "type": "string"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.DiscoveryRunStatus": {
            "type": "string",
            "enum": [
                "running",
                "success",
                "failed"
            ],
            "x-enum-varnames": [
                "DiscoveryRunStatusRunning",
                "DiscoveryRunStatusSuccess",
                "DiscoveryRunStatusFailed"
            ]
        },
        "asset-registration-management-system_backend_internal_model.IMBinding": {
            "type": "object",
            "properties": {
                "createdAt": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "imUserId": {
                    "description": "平台侧用户标识（openId/userId）",
                    "type": "string"
                },
                "platform": {
                    "description": "dingtalk/wecom/feishu",
                    "type": "string"
                },
                "updatedAt": {
                    "type": "string"
                },
                "user": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.User"
                },
                "userId": {
                    "type": "integer"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.InspectionRule": {
            "type": "object",
            "properties": {
                "assignee": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.User"
                },
                "assigneeId": {
                    "description": "巡检执行人",
                    "type": "integer"
                },
                "createdAt": {
                    "type": "string"
                },
                "dayOfMonth": {
                    "description": "monthly：1-31",
                    "type": "integer"
                },
                "dayOfWeek": {
                    "description": "weekly：0=周日 ~ 6=周六",
                    "type": "integer"
                },
                "description": {
                    "description": "巡检内容说明",
                    "type": "string"
                },
                "enabled": {
                    "type": "boolean"
                },
                "frequency": {
                    "description": "daily/weekly/monthly",
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "lastRunAt": {
                    "description": "上次生成巡检工单时间",
                    "type": "string"
                },
                "name": {
                    "description": "规则名称",
                    "type": "string"
                },
                "timeOfDay": {
                    "description": "\"HH:MM\" 执行时间",
                    "type": "string"
                },
                "updatedAt": {
                    "type": "string"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.LicenseAttachment": {
            "type": "object",
            "properties": {
                "contentType": {
                    "type": "string"
                },
                "createdAt": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "licenseId": {
                    "type": "integer"
                },
                "originalName": {
                    "type": "string"
                },
                "size": {
                    "type": "integer"
                },
                "storedName": {
                    "type": "string"
                },
                "uploader": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.User"
                },
                "uploaderId": {
                    "type": "integer"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.Priority": {
            "type": "string",
            "enum": [
                "low",
                "normal",
                "high",
                "urgent"
            ],
            "x-enum-varnames": [
                "PriorityLow",
                "PriorityNormal",
                "PriorityHigh",
                "PriorityUrgent"
            ]
        },
        "asset-registration-management-system_backend_internal_model.Rack": {
            "type": "object",
            "properties": {
                "createdAt": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "remark": {
                    "type": "string"
                },
                "room": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.DatacenterRoom"
                },
                "roomId": {
                    "type": "integer"
                },
                "units": {
                    "description": "U 位数，默认 42",
                    "type": "integer"
                },
                "updatedAt": {
                    "type": "string"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.Role": {
            "type": "string",
            "enum": [
                "admin",
                "asset_manager",
                "approver",
                "applicant"
            ],
            "x-enum-varnames": [
                "RoleAdmin",
                "RoleAssetManager",
                "RoleApprover",
                "RoleApplicant"
            ]
        },
        "asset-registration-management-system_backend_internal_model.SnapshotSource": {
            "type": "string",
            "enum": [
                "discovery",
                "ticket",
                "import",
                "manual"
            ],
            "x-enum-varnames": [
                "SnapshotSourceDiscovery",
                "SnapshotSourceTicket",
                "SnapshotSourceImport",
                "SnapshotSourceManual"
            ]
        },
        "asset-registration-management-system_backend_internal_model.SoftwareLicense": {
            "type": "object",
            "properties": {
                "asset": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Asset"
                },
                "assetId": {
                    "type": "integer"
                },
                "createdAt": {
                    "type": "string"
                },
                "expireDate": {
                    "description": "到期日",
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "description": "软件名",
                    "type": "string"
                },
                "purchaseDate": {
                    "type": "string"
                },
                "remark": {
                    "type": "string"
                },
                "totalSeats": {
                    "description": "授权数量",
                    "type": "integer"
                },
                "type": {
                    "description": "commercial/open-source/subscription/other",
                    "type": "string"
                },
                "updatedAt": {
                    "type": "string"
                },
                "usedSeats": {
                    "description": "已用数量",
                    "type": "integer"
                },
                "vendor": {
                    "description": "厂商",
                    "type": "string"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.StocktakeItem": {
            "type": "object",
            "properties": {
                "asset": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Asset"
                },
                "assetId": {
                    "type": "integer"
                },
                "checkedAt": {
                    "type": "string"
                },
                "createdAt": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "remark": {
                    "type": "string"
                },
                "result": {
                    "description": "pending / matched / missing",
                    "type": "string"
                },
                "taskId": {
                    "type": "integer"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.StocktakeTask": {
            "type": "object",
            "properties": {
                "closedAt": {
                    "type": "string"
                },
                "createdAt": {
                    "type": "string"
                },
                "creator": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.User"
                },
                "creatorId": {
                    "type": "integer"
                },
                "id": {
                    "type": "integer"
                },
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.StocktakeItem"
                    }
                },
                "name": {
                    "type": "string"
                },
                "remark": {
                    "type": "string"
                },
                "status": {
                    "description": "in_progress / closed",
                    "type": "string"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.Ticket": {
            "type": "object",
            "properties": {
                "acceptanceResult": {
                    "type": "string"
                },
                "antivirus": {
                    "type": "string"
                },
                "appVersion": {
                    "type": "string"
                },
                "applicant": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.User"
                },
                "applicantId": {
                    "type": "integer"
                },
                "approver": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.User"
                },
                "approverId": {
                    "type": "integer"
                },
                "archiveNo": {
                    "description": "归档号（关闭归档时生成，草稿为空）",
                    "type": "string"
                },
                "archivedAt": {
                    "type": "string"
                },
                "asset": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Asset"
                },
                "assetId": {
                    "type": "integer"
                },
                "assets": {
                    "description": "关联资产（多资产）",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketAsset"
                    }
                },
                "attachments": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketAttachment"
                    }
                },
                "changeContent": {
                    "type": "string"
                },
                "comments": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketComment"
                    }
                },
                "createdAt": {
                    "type": "string"
                },
                "currentWorkflowStepId": {
                    "type": "integer"
                },
                "currentWorkflowStepName": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "deviceName": {
                    "type": "string"
                },
                "deviceType": {
                    "type": "string"
                },
                "executor": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.User"
                },
                "executorId": {
                    "type": "integer"
                },
                "id": {
                    "type": "integer"
                },
                "impact": {
                    "type": "string"
                },
                "ipAddress": {
                    "type": "string"
                },
                "licenseId": {
                    "description": "关联软件许可（到期自动续费工单专用，可选）",
                    "type": "integer"
                },
                "manufacturer": {
                    "type": "string"
                },
                "openPorts": {
                    "type": "string"
                },
                "priority": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Priority"
                },
                "records": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketRecord"
                    }
                },
                "remark": {
                    "type": "string"
                },
                "result": {
                    "type": "string"
                },
                "runningServices": {
                    "type": "string"
                },
                "slaApprovalDeadline": {
                    "description": "SLA：审批截止时间（submit 时按流程类型写入）",
                    "type": "string"
                },
                "slaCompletionDeadline": {
                    "description": "SLA：执行完成截止时间（start 时写入）",
                    "type": "string"
                },
                "slaOverdueNotified": {
                    "description": "SLA：是否已发送超时通知（防重复）",
                    "type": "boolean"
                },
                "slaStartedAt": {
                    "description": "SLA：进入执行阶段时间（用于剩余时限展示）",
                    "type": "string"
                },
                "status": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketStatus"
                },
                "title": {
                    "type": "string"
                },
                "type": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketType"
                },
                "updatedAt": {
                    "type": "string"
                },
                "workflowSteps": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketWorkflowStep"
                    }
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.TicketAsset": {
            "type": "object",
            "properties": {
                "asset": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Asset"
                },
                "assetId": {
                    "type": "integer"
                },
                "createdAt": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "ticketId": {
                    "type": "integer"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.TicketAttachment": {
            "type": "object",
            "properties": {
                "contentType": {
                    "type": "string"
                },
                "createdAt": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "originalName": {
                    "type": "string"
                },
                "size": {
                    "type": "integer"
                },
                "storedName": {
                    "type": "string"
                },
                "ticketId": {
                    "type": "integer"
                },
                "uploader": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.User"
                },
                "uploaderId": {
                    "type": "integer"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.TicketComment": {
            "type": "object",
            "properties": {
                "actor": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.User"
                },
                "actorId": {
                    "type": "integer"
                },
                "content": {
                    "type": "string"
                },
                "createdAt": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "ticketId": {
                    "type": "integer"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.TicketRecord": {
            "type": "object",
            "properties": {
                "action": {
                    "type": "string"
                },
                "actor": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.User"
                },
                "actorId": {
                    "type": "integer"
                },
                "createdAt": {
                    "type": "string"
                },
                "fromStatus": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketStatus"
                },
                "id": {
                    "type": "integer"
                },
                "remark": {
                    "type": "string"
                },
                "ticketId": {
                    "type": "integer"
                },
                "toStatus": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketStatus"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.TicketStatus": {
            "type": "string",
            "enum": [
                "draft",
                "pending_approval",
                "approved",
                "rejected",
                "in_progress",
                "pending_acceptance",
                "closed",
                "cancelled"
            ],
            "x-enum-varnames": [
                "TicketStatusDraft",
                "TicketStatusPendingApproval",
                "TicketStatusApproved",
                "TicketStatusRejected",
                "TicketStatusInProgress",
                "TicketStatusPendingAcceptance",
                "TicketStatusClosed",
                "TicketStatusCancelled"
            ]
        },
        "asset-registration-management-system_backend_internal_model.TicketType": {
            "type": "string",
            "enum": [
                "asset_register",
                "asset_change",
                "asset_retire",
                "maintenance",
                "inspection",
                "license_renew"
            ],
            "x-enum-comments": {
                "TicketTypeLicenseRenew": "软件许可续费"
            },
            "x-enum-descriptions": [
                "",
                "",
                "",
                "",
                "",
                "软件许可续费"
            ],
            "x-enum-varnames": [
                "TicketTypeAssetRegister",
                "TicketTypeAssetChange",
                "TicketTypeAssetRetire",
                "TicketTypeMaintenance",
                "TicketTypeInspection",
                "TicketTypeLicenseRenew"
            ]
        },
        "asset-registration-management-system_backend_internal_model.TicketWorkflow": {
            "type": "object",
            "properties": {
                "approvalHours": {
                    "description": "SLA：审批时限（小时），nil=不启用",
                    "type": "integer"
                },
                "completionHours": {
                    "description": "SLA：执行完成时限（小时），nil=不启用",
                    "type": "integer"
                },
                "createdAt": {
                    "type": "string"
                },
                "enabled": {
                    "type": "boolean"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "nodes": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketWorkflowNode"
                    }
                },
                "type": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketType"
                },
                "updatedAt": {
                    "type": "string"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.TicketWorkflowNode": {
            "type": "object",
            "properties": {
                "approvers": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketWorkflowNodeApprover"
                    }
                },
                "createdAt": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "sortOrder": {
                    "type": "integer"
                },
                "updatedAt": {
                    "type": "string"
                },
                "workflowId": {
                    "type": "integer"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.TicketWorkflowNodeApprover": {
            "type": "object",
            "properties": {
                "createdAt": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "nodeId": {
                    "type": "integer"
                },
                "user": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.User"
                },
                "userId": {
                    "type": "integer"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.TicketWorkflowStep": {
            "type": "object",
            "properties": {
                "actedAt": {
                    "type": "string"
                },
                "actor": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.User"
                },
                "actorId": {
                    "type": "integer"
                },
                "approvers": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketWorkflowStepApprover"
                    }
                },
                "createdAt": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "remark": {
                    "type": "string"
                },
                "sortOrder": {
                    "type": "integer"
                },
                "status": {
                    "type": "string"
                },
                "ticketId": {
                    "type": "integer"
                },
                "updatedAt": {
                    "type": "string"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.TicketWorkflowStepApprover": {
            "type": "object",
            "properties": {
                "createdAt": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "stepId": {
                    "type": "integer"
                },
                "user": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.User"
                },
                "userId": {
                    "type": "integer"
                }
            }
        },
        "asset-registration-management-system_backend_internal_model.User": {
            "type": "object",
            "properties": {
                "adDn": {
                    "type": "string"
                },
                "authSource": {
                    "type": "string"
                },
                "createdAt": {
                    "type": "string"
                },
                "department": {
                    "type": "string"
                },
                "displayName": {
                    "type": "string"
                },
                "email": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "lastLoginAt": {
                    "type": "string"
                },
                "mustChangePassword": {
                    "description": "首次登录强制改密",
                    "type": "boolean"
                },
                "name": {
                    "type": "string"
                },
                "proxyUserId": {
                    "description": "审批代理：设置为本人审批的工单由代理处理",
                    "type": "integer"
                },
                "role": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Role"
                },
                "status": {
                    "type": "string"
                },
                "updatedAt": {
                    "type": "string"
                },
                "username": {
                    "type": "string"
                }
            }
        },
        "asset-registration-management-system_backend_internal_service.ADUserInfo": {
            "type": "object",
            "properties": {
                "department": {
                    "type": "string"
                },
                "displayName": {
                    "type": "string"
                },
                "dn": {
                    "type": "string"
                },
                "email": {
                    "type": "string"
                },
                "username": {
                    "type": "string"
                }
            }
        },
        "asset-registration-management-system_backend_internal_service.BackupContent": {
            "type": "object",
            "properties": {
                "path": {
                    "type": "string"
                },
                "sha256": {
                    "type": "string"
                },
                "size": {
                    "type": "integer"
                }
            }
        },
        "asset-registration-management-system_backend_internal_service.BackupInfo": {
            "type": "object",
            "properties": {
                "contents": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/asset-registration-management-system_backend_internal_service.BackupContent"
                    }
                },
                "encrypted": {
                    "type": "boolean"
                },
                "modTime": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "offsiteCopied": {
                    "type": "boolean"
                },
                "sha256": {
                    "type": "string"
                },
                "size": {
                    "type": "integer"
                },
                "verifiedAt": {
                    "type": "string"
                },
                "verifyError": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.SLASummary": {
            "type": "object",
            "properties": {
                "applicable": {
                    "type": "boolean"
                },
                "met": {
                    "type": "integer"
                },
                "overdue": {
                    "type": "integer"
                },
                "rate": {
                    "description": "达标率 0-100",
                    "type": "number"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "internal_httpapi.TicketStats": {
            "type": "object",
            "properties": {
                "byPriority": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/internal_httpapi.ticketGroupedCounts"
                    }
                },
                "byStatus": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/internal_httpapi.ticketGroupedCounts"
                    }
                },
                "byType": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/internal_httpapi.ticketGroupedCounts"
                    }
                },
                "monthlyTrend": {
                    "description": "近 12 个月（按创建月份）",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/internal_httpapi.ticketGroupedCounts"
                    }
                },
                "slaSummary": {
                    "$ref": "#/definitions/internal_httpapi.SLASummary"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "internal_httpapi.adConfigRequest": {
            "type": "object",
            "required": [
                "baseDn",
                "bindDn",
                "ldapUrl"
            ],
            "properties": {
                "advancedFilter": {
                    "type": "boolean"
                },
                "baseDn": {
                    "type": "string"
                },
                "bindDn": {
                    "type": "string"
                },
                "bindPassword": {
                    "type": "string"
                },
                "enabled": {
                    "type": "boolean"
                },
                "excludeDisabled": {
                    "type": "boolean"
                },
                "filterUserObject": {
                    "type": "boolean"
                },
                "ldapUrl": {
                    "type": "string"
                },
                "loginAttribute": {
                    "type": "string"
                },
                "userFilter": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.adImportRequest": {
            "type": "object",
            "required": [
                "username"
            ],
            "properties": {
                "role": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Role"
                },
                "status": {
                    "type": "string"
                },
                "username": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.adLookupRequest": {
            "type": "object",
            "required": [
                "username"
            ],
            "properties": {
                "username": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.assetRequest": {
            "type": "object",
            "required": [
                "ip"
            ],
            "properties": {
                "additionalIPs": {
                    "type": "string"
                },
                "appVersion": {
                    "type": "string"
                },
                "assetNo": {
                    "type": "string"
                },
                "assetType": {
                    "type": "string"
                },
                "businessSystem": {
                    "type": "string"
                },
                "cpu": {
                    "type": "string"
                },
                "department": {
                    "type": "string"
                },
                "disk": {
                    "type": "string"
                },
                "environment": {
                    "type": "string"
                },
                "hostname": {
                    "type": "string"
                },
                "ip": {
                    "type": "string"
                },
                "location": {
                    "type": "string"
                },
                "macAddress": {
                    "type": "string"
                },
                "maintenanceVendor": {
                    "type": "string"
                },
                "managementIp": {
                    "type": "string"
                },
                "manufacturer": {
                    "type": "string"
                },
                "memory": {
                    "type": "string"
                },
                "model": {
                    "type": "string"
                },
                "onlineDate": {
                    "type": "string"
                },
                "openPorts": {
                    "type": "string"
                },
                "os": {
                    "type": "string"
                },
                "osVersion": {
                    "type": "string"
                },
                "owner": {
                    "type": "string"
                },
                "purchaseDate": {
                    "type": "string"
                },
                "rack": {
                    "type": "string"
                },
                "rackPosition": {
                    "type": "string"
                },
                "remark": {
                    "type": "string"
                },
                "runningServices": {
                    "type": "string"
                },
                "sequenceNo": {
                    "type": "string"
                },
                "serialNo": {
                    "type": "string"
                },
                "status": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.AssetStatus"
                },
                "subnet": {
                    "type": "string"
                },
                "warrantyExpireDate": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.batchDeleteRequest": {
            "type": "object",
            "required": [
                "ids"
            ],
            "properties": {
                "ids": {
                    "type": "array",
                    "items": {
                        "type": "integer"
                    }
                }
            }
        },
        "internal_httpapi.changePasswordDocRequest": {
            "type": "object",
            "properties": {
                "newPassword": {
                    "type": "string"
                },
                "oldPassword": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.createStocktakeRequest": {
            "type": "object",
            "required": [
                "name"
            ],
            "properties": {
                "assetType": {
                    "description": "可选：按资产类型过滤盘点范围，空=全部",
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "remark": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.credentialRequest": {
            "type": "object",
            "required": [
                "name",
                "username"
            ],
            "properties": {
                "assetId": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "remark": {
                    "type": "string"
                },
                "secret": {
                    "type": "string"
                },
                "type": {
                    "type": "string"
                },
                "username": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.discoveryHostActionRequest": {
            "type": "object",
            "required": [
                "hostIds"
            ],
            "properties": {
                "hostIds": {
                    "type": "array",
                    "items": {
                        "type": "integer"
                    }
                }
            }
        },
        "internal_httpapi.discoveryRuleRequest": {
            "type": "object",
            "required": [
                "name",
                "targets"
            ],
            "properties": {
                "autoAdopt": {
                    "type": "boolean"
                },
                "autoApply": {
                    "type": "boolean"
                },
                "autoTicket": {
                    "type": "boolean"
                },
                "enabled": {
                    "type": "boolean"
                },
                "incremental": {
                    "type": "boolean"
                },
                "intervalMinutes": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "ports": {
                    "type": "string"
                },
                "probePorts": {
                    "type": "string"
                },
                "scanWindowEnd": {
                    "type": "string"
                },
                "scanWindowStart": {
                    "type": "string"
                },
                "serviceDetect": {
                    "type": "boolean"
                },
                "targets": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.discoveryTrendItem": {
            "type": "object",
            "properties": {
                "changed": {
                    "description": "变更主机数",
                    "type": "integer"
                },
                "date": {
                    "description": "YYYY-MM-DD",
                    "type": "string"
                },
                "new": {
                    "description": "新增主机数",
                    "type": "integer"
                },
                "offline": {
                    "description": "离线主机数",
                    "type": "integer"
                },
                "online": {
                    "description": "恢复在线主机数",
                    "type": "integer"
                },
                "runs": {
                    "description": "成功运行次数",
                    "type": "integer"
                }
            }
        },
        "internal_httpapi.errorResponse": {
            "type": "object",
            "properties": {
                "error": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.imBindingRequest": {
            "type": "object",
            "required": [
                "imUserId",
                "platform",
                "userId"
            ],
            "properties": {
                "imUserId": {
                    "type": "string"
                },
                "platform": {
                    "type": "string"
                },
                "userId": {
                    "type": "integer"
                }
            }
        },
        "internal_httpapi.imCallbackConfigRequest": {
            "type": "object",
            "properties": {
                "appSecret": {
                    "type": "string"
                },
                "corpId": {
                    "type": "string"
                },
                "enabled": {
                    "type": "boolean"
                },
                "encodingAESKey": {
                    "type": "string"
                },
                "platform": {
                    "type": "string"
                },
                "token": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.imConfigRequest": {
            "type": "object",
            "properties": {
                "enabled": {
                    "type": "boolean"
                },
                "platform": {
                    "type": "string"
                },
                "secret": {
                    "type": "string"
                },
                "webhook": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.inspectionRuleRequest": {
            "type": "object",
            "required": [
                "assigneeId",
                "frequency",
                "name",
                "timeOfDay"
            ],
            "properties": {
                "assigneeId": {
                    "type": "integer"
                },
                "dayOfMonth": {
                    "type": "integer"
                },
                "dayOfWeek": {
                    "type": "integer"
                },
                "description": {
                    "type": "string"
                },
                "enabled": {
                    "type": "boolean"
                },
                "frequency": {
                    "description": "daily/weekly/monthly",
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "timeOfDay": {
                    "description": "\"HH:MM\"",
                    "type": "string"
                }
            }
        },
        "internal_httpapi.loginRequest": {
            "type": "object",
            "required": [
                "password",
                "username"
            ],
            "properties": {
                "password": {
                    "type": "string"
                },
                "username": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.mailConfigRequest": {
            "type": "object",
            "properties": {
                "enabled": {
                    "type": "boolean"
                },
                "fromAddress": {
                    "type": "string"
                },
                "fromName": {
                    "type": "string"
                },
                "password": {
                    "type": "string"
                },
                "smtpHost": {
                    "type": "string"
                },
                "smtpPort": {
                    "type": "integer"
                },
                "startTls": {
                    "type": "boolean"
                },
                "useTls": {
                    "type": "boolean"
                },
                "username": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.mailTestRequest": {
            "type": "object",
            "properties": {
                "recipient": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.rackRequest": {
            "type": "object",
            "required": [
                "name",
                "roomId"
            ],
            "properties": {
                "name": {
                    "type": "string"
                },
                "remark": {
                    "type": "string"
                },
                "roomId": {
                    "type": "integer"
                },
                "units": {
                    "type": "integer"
                }
            }
        },
        "internal_httpapi.reauthenticateDocRequest": {
            "type": "object",
            "properties": {
                "password": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.roomRequest": {
            "type": "object",
            "required": [
                "name"
            ],
            "properties": {
                "location": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "remark": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.serviceStat": {
            "type": "object",
            "properties": {
                "count": {
                    "type": "integer"
                },
                "port": {
                    "type": "string"
                },
                "service": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.softwareLicenseRequest": {
            "type": "object",
            "required": [
                "name"
            ],
            "properties": {
                "assetId": {
                    "type": "integer"
                },
                "expireDate": {
                    "type": "string"
                },
                "licenseKey": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "purchaseDate": {
                    "type": "string"
                },
                "remark": {
                    "type": "string"
                },
                "totalSeats": {
                    "type": "integer"
                },
                "type": {
                    "type": "string"
                },
                "usedSeats": {
                    "type": "integer"
                },
                "vendor": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.stocktakeItemRequest": {
            "type": "object",
            "required": [
                "result"
            ],
            "properties": {
                "remark": {
                    "type": "string"
                },
                "result": {
                    "description": "matched / missing",
                    "type": "string"
                }
            }
        },
        "internal_httpapi.subnetStat": {
            "type": "object",
            "properties": {
                "count": {
                    "type": "integer"
                },
                "subnet": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.ticketActionRequest": {
            "type": "object",
            "properties": {
                "acceptanceResult": {
                    "type": "string"
                },
                "remark": {
                    "type": "string"
                },
                "result": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.ticketArchiveBatchRequest": {
            "type": "object",
            "required": [
                "ids"
            ],
            "properties": {
                "ids": {
                    "type": "array",
                    "items": {
                        "type": "integer"
                    }
                }
            }
        },
        "internal_httpapi.ticketCommentRequest": {
            "type": "object",
            "required": [
                "content"
            ],
            "properties": {
                "content": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.ticketGroupedCounts": {
            "type": "object",
            "properties": {
                "count": {
                    "type": "integer"
                },
                "label": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.ticketRequest": {
            "type": "object",
            "required": [
                "title",
                "type"
            ],
            "properties": {
                "antivirus": {
                    "type": "string"
                },
                "appVersion": {
                    "type": "string"
                },
                "assetId": {
                    "type": "integer"
                },
                "assetIds": {
                    "description": "关联资产（多资产），优先于 assetId",
                    "type": "array",
                    "items": {
                        "type": "integer"
                    }
                },
                "changeContent": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "deviceName": {
                    "type": "string"
                },
                "deviceType": {
                    "type": "string"
                },
                "impact": {
                    "type": "string"
                },
                "ipAddress": {
                    "type": "string"
                },
                "manufacturer": {
                    "type": "string"
                },
                "openPorts": {
                    "type": "string"
                },
                "priority": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Priority"
                },
                "remark": {
                    "type": "string"
                },
                "runningServices": {
                    "type": "string"
                },
                "title": {
                    "type": "string"
                },
                "type": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.TicketType"
                }
            }
        },
        "internal_httpapi.userRequest": {
            "type": "object",
            "required": [
                "name",
                "role",
                "username"
            ],
            "properties": {
                "authSource": {
                    "type": "string"
                },
                "department": {
                    "type": "string"
                },
                "displayName": {
                    "type": "string"
                },
                "email": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "password": {
                    "type": "string"
                },
                "proxyUserId": {
                    "type": "integer"
                },
                "role": {
                    "$ref": "#/definitions/asset-registration-management-system_backend_internal_model.Role"
                },
                "status": {
                    "type": "string"
                },
                "username": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.workflowNodeRequest": {
            "type": "object",
            "required": [
                "name"
            ],
            "properties": {
                "approverIds": {
                    "description": "可为空：提交工单时按资产负责人/类型默认审批人自动分派",
                    "type": "array",
                    "items": {
                        "type": "integer"
                    }
                },
                "name": {
                    "type": "string"
                }
            }
        },
        "internal_httpapi.workflowRequest": {
            "type": "object",
            "required": [
                "nodes"
            ],
            "properties": {
                "approvalHours": {
                    "description": "SLA 审批时限（小时），空=不启用",
                    "type": "integer"
                },
                "completionHours": {
                    "description": "SLA 执行完成时限（小时），空=不启用",
                    "type": "integer"
                },
                "enabled": {
                    "type": "boolean"
                },
                "name": {
                    "type": "string"
                },
                "nodes": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/internal_httpapi.workflowNodeRequest"
                    }
                }
            }
        }
    },
    "securityDefinitions": {
        "BearerAuth": {
            "type": "apiKey",
            "name": "Authorization",
            "in": "header"
        }
    }
}`

// SwaggerInfo holds exported Swagger Info so clients can modify it
var SwaggerInfo = &swag.Spec{
	Version:          "1.0",
	Host:             "",
	BasePath:         "/api/v1",
	Schemes:          []string{},
	Title:            "Asset Registration Management System API",
	Description:      "企业内部服务器资产台账与工单流程管理系统 REST API。",
	InfoInstanceName: "swagger",
	SwaggerTemplate:  docTemplate,
	LeftDelim:        "{{",
	RightDelim:       "}}",
}

func init() {
	swag.Register(SwaggerInfo.InstanceName(), SwaggerInfo)
}
